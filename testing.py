...
# Сложи тук твоя DATABASE_URL от Railway (препоръчително е да е в .env файл)
DB_URL = "postgresql://postgres:IZqVUkqAdSQZbnXqpWEVumimZLFRtPFW@hopper.proxy.rlwy.net:16266/railway"
...
def sign_up(email, password, profile_pic_url=None):
    # 1. Хешираме паролата (НИКОГА не я пази в чист текст!)
    salt = bcrypt.gensalt()
    ...
    try:
        ...
        query = """
            ...
            VALUES (%s, %s, %s)
        """
        cur.execute(query, (email, hashed_pw, profile_pic_url))

        conn.commit()
        print(f"✅ Акаунтът {email} е създаден успешно!")

    except psycopg2.IntegrityError:
        print("❌ Грешка: Потребител с този имейл вече съществува.")
    except Exception as e:
...
def login(email, password):
    try:
        ...
        if result:
            ...
            # 2. Проверяваме дали въведената парола съвпада с хеша
            if bcrypt.checkpw(password.encode("utf-8"), stored_hash.encode("utf-8")):
                print(f"🔓 Успешен вход! Добре дошъл, {email}.")
                return True
            else:
                print("❌ Грешна парола.")
        else:
...
# --- ТЕСТВАНЕ ---
# sign_up("konstantinviolinov@outlook.com", "kv0889909595")
login("konstantinviolinov@outlook.com", "kv0889909595")
