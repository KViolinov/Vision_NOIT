...
@require_login
def recognize_song():
    generate_audio_from_text(text="Няма проблем, дайте ми само една секунда.", voice=jarvis_voice)

    time.sleep(1)  # Pause for a moment to ensure the message is delivered

    generate_audio_from_text(text="Готов съм, доближете телефона до микрофона.", voice=jarvis_voice)
    ...
    if "track" in result:
        ...
        generate_audio_from_text(text=f"Песента е {title} от {artist}. Желаете ли да я пусна в spotify?", voice=jarvis_voice)
